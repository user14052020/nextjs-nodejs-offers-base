...
export class WorksService implements OnModuleInit {
  ...
  async create(dto: CreateWorkDto) {
    const created = await this.uow.withTransaction(async (session) => {
      ...
      const creditedAmount = this.normalizeCreditedAmount(dto.creditedAmount, amount);
  ...
  async update(id: string, dto: UpdateWorkDto) {
    const updated = await this.uow.withTransaction(async (session) => {
      ...
      const currentCreditedAmount = this.normalizeCreditedAmount(
        (current as { creditedAmount?: number }).creditedAmount,
        currentAmount
      );
      ...
      if (dto.items) {
        const items = this.normalizeItems(dto.items);
        payload.items = items;
        nextAmount = this.calculateTotal(items);
        payload.amount = nextAmount;
      }
      payload.creditedAmount =
        dto.creditedAmount !== undefined
          ? this.normalizeCreditedAmount(dto.creditedAmount, nextAmount)
          : dto.items && this.areAmountsEqual(currentCreditedAmount, currentAmount)
            ? nextAmount
  ...
  private buildActPdf(work: any, organization: any, client: any): Promise<Buffer> {
    return new Promise((resolve) => {
      ...
      for (const item of items) {
        const itemRowHeight = this.calculatePdfTableRowHeight(
          doc,
          fonts,
          [
            { text: item.name, width: colWidths[0] - 16 },
            { text: this.formatQuantity(item.quantity), width: colWidths[1] - rightPadding },
            { text: this.formatAmount(item.price), width: colWidths[2] - rightPadding },
            { text: this.formatAmount(item.amount), width: colWidths[3] - rightPadding }
          ],
          minItemRowHeight,
          BODY_FONT_SIZE,
          TABLE_CELL_PADDING
    ...
    return new Promise((resolve) => {
      ...
      items.forEach((item, index) => {
        const itemRowHeight = this.calculatePdfTableRowHeight(
          ...
          fonts,
          [
            { text: String(index + 1), width: invoiceCols[0] },
            { text: item.name, width: invoiceCols[1] - 16 },
            { text: this.formatAmount(item.amount), width: invoiceCols[2] - 12 }
          ],
          minItemRowHeight,
          BODY_FONT_SIZE,
          TABLE_CELL_PADDING
  ...
  private calculatePdfTableRowHeight(
    doc: PDFKit.PDFDocument,
    fonts: PdfFonts,
    cells: PdfTableMeasurementCell[],
    minHeight: number,
    fontSize: number,
    verticalPadding: number
  ): number {
    const contentHeight = cells.reduce(
      (maxHeight, cell) => Math.max(maxHeight, this.measurePdfTextHeight(doc, fonts, cell, fontSize)),
      0
    );
    return Math.ceil(Math.max(minHeight, contentHeight + verticalPadding * 2));
  }

  private measurePdfTextHeight(
    doc: PDFKit.PDFDocument,
    fonts: PdfFonts,
    cell: PdfTableMeasurementCell,
    fontSize: number
  ): number {
    this.applyFont(doc, fonts, cell.bold ? 'bold' : 'regular');
    doc.fontSize(fontSize);
    return doc.heightOfString(cell.text, { width: Math.max(cell.width, 1) });
  }
  ...
  private normalizeCreditedAmount(value: unknown, fallback: number): number {
    const safeFallback = Number.isFinite(fallback) && fallback >= 0 ? fallback : 0;
    if (value === undefined || value === null || value === '') {
      return safeFallback;
    }

    const creditedAmount = Number(value);
    if (!Number.isFinite(creditedAmount)) {
      throw new ValidationServiceException('Сумма зачисления указана некорректно');
    }

    if (creditedAmount < 0) {
      throw new ValidationServiceException('Сумма зачисления не может быть отрицательной');
    }
