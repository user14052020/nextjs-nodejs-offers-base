...
import { Alert, Button, Group, Paper, Stack, Switch, Table, Text, Title } from '@mantine/core';
...
export const WorksTable: React.FC<{
  ...
  const [paymentLoadingId, setPaymentLoadingId] = React.useState<string | null>(null);
  const [paymentOverrides, setPaymentOverrides] = React.useState<Map<string, boolean>>(new Map());
  ...
  const handlePaymentChange = async (work: Work, isPayed: boolean) => {
    const previousValue = paymentOverrides.get(work._id) ?? Boolean(work.isPayed);
    setError(null);
    setPaymentLoadingId(work._id);
    setPaymentOverrides((prev) => {
      const next = new Map(prev);
      next.set(work._id, isPayed);
      return next;
    });

    try {
      await updateWork(work._id, { isPayed });
      await onChange();
      setPaymentOverrides((prev) => {
        const next = new Map(prev);
        next.delete(work._id);
        return next;
      });
    } catch (err) {
      setPaymentOverrides((prev) => {
        const next = new Map(prev);
        next.set(work._id, previousValue);
        return next;
      });
      setError(err instanceof Error ? err.message : 'Ошибка обновления оплаты');
    } finally {
        ...
        <Table striped highlightOnHover withTableBorder withColumnBorders>
          ...
          <Table.Tbody>
            {items.map((work) => {
              ...
              const isPayed = paymentOverrides.get(work._id) ?? Boolean(work.isPayed);
              ...
              return (
                <Table.Tr key={work._id}>
                  ...
                  <Table.Td>
                    {formatAmount(work.creditedAmount ?? work.amount)} {work.currency || 'RUB'}
                  </Table.Td>
                  <Table.Td>
                    <Switch
                      checked={isPayed}
                      ...
                      disabled={paymentLoadingId === work._id}
                      onChange={(event) => handlePaymentChange(work, event.currentTarget.checked)}
                    />
                  </Table.Td>
                  <Table.Td>
