...
import { Button, Checkbox, Group, Paper, Select, SimpleGrid, Stack, Text, TextInput, Title } from '@mantine/core';
...
const areAmountsEqual = (left: number, right: number) => Math.abs(left - right) < 0.000001;

const createEmptyForm = (): WorkFormState => ({
  items: [emptyItem()],
  creditedAmount: '',
  isPayed: false,
  currency: 'RUB',
...
const getCreditedAmountFromWork = (work: Work) =>
  typeof work.creditedAmount === 'number' && Number.isFinite(work.creditedAmount) ? work.creditedAmount : work.amount;
...
export const WorkForm: React.FC<{
  ...
  const [form, setForm] = React.useState<WorkFormState>(createEmptyForm);
  const [loading, setLoading] = React.useState(false);
  const [creditedAmountManual, setCreditedAmountManual] = React.useState(false);

  React.useEffect(() => {
    if (!editingItem) {
      setForm(createEmptyForm());
      setCreditedAmountManual(false);
      return;
    }
    ...
    const creditedAmount = getCreditedAmountFromWork(editingItem);

    setForm({
      items: normalizeItemFromWork(editingItem),
      creditedAmount: String(creditedAmount),
      isPayed: Boolean(editingItem.isPayed),
    ...
    setCreditedAmountManual(!areAmountsEqual(creditedAmount, editingItem.amount));
  ...
  React.useEffect(() => {
    if (creditedAmountManual) {
      return;
    }

    const nextCreditedAmount = String(totalAmount);
    setForm((prev) => (prev.creditedAmount === nextCreditedAmount ? prev : { ...prev, creditedAmount: nextCreditedAmount }));
  }, [totalAmount, creditedAmountManual]);

  const handleCreditedAmountChange = (value: string) => {
    const normalizedValue = value.replace(',', '.');
    const numeric = Number(normalizedValue);
    const shouldAutoFill =
      normalizedValue.trim() === '' || (Number.isFinite(numeric) && areAmountsEqual(numeric, totalAmount));

    setCreditedAmountManual(!shouldAutoFill);
    handleChange('creditedAmount', normalizedValue);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const payload = {
        items: payloadItems,
        creditedAmount: form.creditedAmount.trim() === '' ? totalAmount : parsePositiveNumber(form.creditedAmount),
        isPayed: form.isPayed,
    ...
    <Paper withBorder shadow="sm" radius="lg" p="xl" component="form" onSubmit={handleSubmit}>
      <Stack gap="md">
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
          ...
          <Field label="Сумма документа">
            <TextInput value={formatAmount(totalAmount)} readOnly />
          </Field>
          <Field label="Сумма зачисления">
            <TextInput
              ...
              onChange={(event) => handleCreditedAmountChange(event.currentTarget.value)}
            />
          </Field>
          <Field label="Оплата">
            <Checkbox
              ...
              onChange={(event) => setForm((prev) => ({ ...prev, isPayed: event.currentTarget.checked }))}
            />
          </Field>
        </SimpleGrid>
        ...
        <Stack gap="sm">
          ...
          <Text size="sm" c="dimmed">
            Итого по позициям: {formatAmount(totalAmount)} RUB. Сумма зачисления по умолчанию равна сумме документа,
            но ее можно скорректировать вручную.
          </Text>
        </Stack>
