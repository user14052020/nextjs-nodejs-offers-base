...
MongoDB, Redis и Elasticsearch в базовом сценарии доступны только внутри Docker-сети. Это сделано специально, чтобы запуск на сервере не конфликтовал с уже занятыми `27017`, `6379` и `9200`.

Если нужен доступ к этим сервисам с хоста, запускайте с дополнительным override-файлом:

```bash
docker compose -f docker-compose.yml -f docker-compose.host-ports.yml up --build
```

Если порты на хосте уже заняты, поменяйте `MONGO_PORT`, `REDIS_PORT` и `ELASTICSEARCH_PORT` в `.env`.
...
`MONGO_PORT`, `REDIS_PORT` и `ELASTICSEARCH_PORT` используются только если вы запускаете compose с файлом `docker-compose.host-ports.yml`.
